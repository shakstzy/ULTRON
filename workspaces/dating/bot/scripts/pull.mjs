...
import { findEntityByMatchId, recordRedirect, clearRedirects } from "../src/runtime/entity-store.mjs";
...
try {
  ...
  for (const m of matches.slice(0, limit)) {
    try {
      ...
      opened += 1;
      // Two-strike auto-archive on thread redirect (likely unmatch). Successful
      // resolve clears the counter.
      if (r.url_redirected) {
        const ent = await findEntityByMatchId(m.matchId);
        if (ent) await recordRedirect(ent.slug);
      } else if (r.slug) {
        await clearRedirects(r.slug);
      }
      ...
      console.log(`thread ${m.name || "?"} -> ${r.slug || (r.url_redirected ? "(redirected)" : "?")} (${r.messages_new}/${r.messages_total} new)${diffStr}`);
    } catch (e) {
