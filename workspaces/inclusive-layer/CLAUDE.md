...
# Inclusive Layer — Workspace Router

You are in workspace `inclusive-layer` — Adithya's past growth-lead contract with Inclusive Layer.

## What this workspace is

A multi-month contract Adithya took as growth lead earlier in 2025. Drive content lived under `adithya@outerscope.xyz` in the `INCLUSIVELAYER` folder. Now an archive workspace — historical reference, occasional follow-up email, the people who threaded through it.
...
4. Commit messages: `chore(inclusive-layer): <stage> <YYYY-MM-DD>`.
5. Do NOT mix Inclusive Layer content with `mosaic`, `synapse`, `eclipse`, or `sei`. Cross-references via `[[@person]]` global stubs only.

## Routing table — common queries

| Query | Path |
|---|---|
| Who is X (Inclusive Layer team / counterparty)? | `wiki/entities/people/<x>.md` then `[[@x]]` |
| What was the engagement scope? | `wiki/synthesis/engagement.md` |
| Drive content from the contract? | `raw/drive/adithya-outerscope/INCLUSIVELAYER/...` |
| Granola notes for Inclusive Layer meetings? | `raw/granola/<YYYY-MM>/...` (if folder exists upstream) |
...
Declared in `config/sources.yaml`. Cross-source routing in `_shell/docs/source-routing.md`. Sources today: drive (`INCLUSIVELAYER` folder under `adithya@outerscope.xyz`), gmail (label TBD), granola (folder TBD), manual deposits.
