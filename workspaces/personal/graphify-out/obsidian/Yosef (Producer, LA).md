---
source_file: "raw/imessage/individuals/yosef-producer-la/2025/2025-09__yosef-producer-la.md"
type: "document"
community: "Community 10"
tags:
  - graphify/document
  - graphify/EXTRACTED
  - community/Community_10
---

# Yosef (Producer, LA)

## Connections
- [[Adithya]] - `messages_with` [EXTRACTED]
- [[Caitlin (Prescription Studio contact)]] - `mentioned_by` [EXTRACTED]
- [[Donny (Producer LA)]] - `co_attended` [INFERRED]
- [[Louie]] - `co_attended` [EXTRACTED]
- [[Music Production]] - `works_at` [INFERRED]
- [[Nati (Yosef's girlfriend)]] - `related_to` [EXTRACTED]
- [[Prescription (Recording Studio, LA)]] - `discussed_with` [EXTRACTED]
- [[Tyler  KW  Kuji (Producers)]] - `works_at` [INFERRED]
- [[Tyler Sheppard (LA, music producer)]] - `co_attended` [EXTRACTED]

#graphify/document #graphify/EXTRACTED #community/Community_10